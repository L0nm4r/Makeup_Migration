...
session_start();
  ...
  if (!file_exists('usermakeup')){
    mkdir('usermakeup');
  }
  //为上传的文件新起一个名字，保证更加安全
  $new_filename = date('YmdHis',time()).rand(100,1000).'.'.$ext_suffix;
  //将文件从临时路径移动到磁盘
  if (move_uploaded_file($temp_name, 'usermakeup/'.$new_filename)){
    $_SESSION['usermakeup']=$new_filename;
    echo "<script>window.location.href='./pingfen.php';</script>";
  }else{
    echo "<script>alert('文件上传失败,错误码：$error');window.history.go(-1);</script>";
  }
